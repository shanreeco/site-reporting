...
export default function App(){
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      <header className="px-6 pt-6 border-b bg-white">
      ...
      <main className="p-6 max-w-7xl mx-auto">
        <p className="text-neutral-700">Dashboard content will go here (logs, charts, etc).</p>
      </main>
    </div>
  );
}
...
function toCSV(rows, headerOrder){
  ...
  const esc = (v) => {
    ...
    // Use template literals for easier reading and to avoid quote issues
    if (s.includes(',') || s.includes(NL) || s.includes('"')) {
      return `"${s.replaceAll('"', '""')}"`;
    }
    return s;
  };
